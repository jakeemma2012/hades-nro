...
public abstract class Boss extends Player implements BossInterface {
        ...
        try {
            ...
            if (Util.canDoWithTime(lastTimeUpdate, 60000)) {
                if (timeAppear >= TIME_RESEND_LOCATION) {
                    if (this.zone != null && !(this instanceof BossBanDoKhoBau) && !(this instanceof BossMabuWar)
                            && !(this instanceof BossYardart)) {
    ...
    public void dropItemReward(int tempId, int playerId, int... quantity) {
        if (!this.zone.map.isMapOffline && this.zone.map.type == ConstMap.MAP_NORMAL) {
            ...
            if (itemMap.itemTemplate.id == 568) {
    ...
    public boolean generalRewards(Player player, int maxLevel, int ratio) {
        if (player != null) {
            ...
            if (Util.isTrue(ratio, 100)) {
                ...
                int typeTrangBi = Util.nextInt(0, 3);
                if (Manager.QUANTITY_NTL < 10) {
                    if (Util.isTrue(1, 100)) {
                        typeTrangBi = 4;
                        Manager.QUANTITY_NTL++;
                    }
                }
                short itemId = ConstItem.doSKHVip[typeTrangBi][Util.nextInt(0, 2)][level];
                itemMap2 = new ItemMap(this.zone, itemId, 1, x, y,
                        player.id);
                RewardService.gI().RewardBoss(itemMap2);
    ...
    public boolean generalRewardsOmega(Player player, int maxLevel, int ratio, int raTioNhan) {
        if (player != null) {
            ItemMap itemMap2 = null;
            if (Util.isTrue(ratio, 100)) {
                int level = maxLevel;
                ...
                int y = player.zone.map.yPhysicInTop(x, player.location.y - 24);
                int typeTrangBi = Util.nextInt(0, 3);
                if (Manager.QUANTITY_NTL < 5) {
                    if (Util.isTrue(raTioNhan, 100)) {
                        typeTrangBi = 4;
                        Manager.QUANTITY_NTL++;
                    }
                }
    ...
    public boolean generalRewards(Player player, int maxLevel) {
        if (player != null) {
            ItemMap itemMap2 = null;
            int level = Util.nextInt(8, maxLevel);
            if (level > 13) {
                level = 13;
            }
            int x = player.location.x;
    ...
    public void baseRewards(Player player, int min_count, int max_count, int type) {
        if (player != null) {
            int count = Util.nextInt(min_count, max_count);
            for (int i = 0; i < count; i++) {
                ...
                int y = this.zone.map.yPhysicInTop(x, this.location.y - 24);
                itemMap = ItemService.gI().BaseRewar(zone, player, x, y, (byte) type);
                Service.getInstance().dropItemMap(zone, itemMap);
            }
    ...
    public void getRewardsBossNormalOrKichAn(Boss boss, Player player, int tile, int titleRate) {
        if (boss != null && player != null) {
            ItemMap itemMap = ArrietyDrop.DropItemRewardNormal(player, 1, this.location.x,
                    this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24));
            if (Util.isTrue(tile, titleRate)) {
                ArrietyDrop.initialOptionAnKichHoat(itemMap);
            }
            Service.getInstance().dropItemMap(zone, itemMap);
        }
    }

    public void getRewardGranola(Boss boss, Player player, int tile, int titleRate, int type) {
        if (boss != null && player != null) {
            if (Util.isTrue(tile, titleRate)) {
                ItemMap itm = null;
                switch (type) {
                    case 0:
                        itm = new ItemMap(zone,
                                ConstItem.LIST_ITEM_CLOTHES[Util.nextInt(0, 2)][Util.nextInt(0, 4)][Util.getOne(11,
                                        12)],
                                1, this.location.x,
