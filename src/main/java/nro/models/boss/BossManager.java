...
public class BossManager {
    ...
    public void updateAllBoss() {
        List<Boss> bossesCopy = new ArrayList<>(BOSSES_IN_GAME);
        for (int i = bossesCopy.size() - 1; i >= 0; i--) {
            try {
                Boss boss = bossesCopy.get(i);
                if (boss != null) {
                    boss.update();
                }
            } catch (Exception e) {
    ...
    public void showListBoss(Player player) {
        try {
            long aliveBossCount = BOSSES_IN_GAME.stream().filter(boss -> boss != null)
                    .filter(boss -> !BossFactory.isYar((byte) boss.id)).count();
