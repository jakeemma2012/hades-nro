            ...
            switch (select) {
                case 0:
                    ...
                    if (dongPi == null || dongPi.quantity < CLEAN_COST) {
                        Service.getInstance().sendThongBao(player, "Bạn không có đủ " + CLEAN_COST + " "
                                + (dongPi != null ? dongPi.getName() : "đồng pi") + " để tham chiến.");
                        return;
                    }
                ...
                case 1:
                    ...
                    if (this.last_time_summon_boss != null && duration.toMinutes() < 5) {
                        ...
                        Service.getInstance().sendThongBao(player,
                                "Cần chờ " + phut + " phút và " + giay + " giây để có thể tiếp tục gọi boss");
                        return;
                    }
                    if (pi == null || pi.quantity < SUMON_COST) {
                        Service.getInstance().sendThongBao(player, "Bạn không có đủ " + SUMON_COST + " "
                                + (pi != null ? pi.getName() : "đồng pi") + " để tham chiến.");
                        return;
                    }
                    String thongbao = "Bạn đã gọi Boss thành công ở Map: " + player.zone.map.mapName + " khu: "
                            + player.zone.zoneId;
