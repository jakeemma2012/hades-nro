...
public class TruongLaoGuru extends Npc {
    ...
    public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
            ...
            // case 0:
            // TranhNgoc tn =
            // ServerManager.gI().getTranhNgocManager().findByPLayerId(player.id);
            // if (TranhNgoc.isTimeRegWar()) {
            // String message = "Ngọc rồng Namếc đang bị 2 thế lực tranh giành\nHãy chọn cấp
            // độ tham gia tùy theo sức mạnh bản thân";
            // if (tn == null) {
            ...
            // tham gia tùy theo sức mạnh bản thân";
            // } else {
            // message = "Ngọc rồng Namếc đang bị 2 thế lực tranh giành\nHãy chọn cấp độ
            // tham gia tùy theo sức mạnh bản thân\nPhe Cadic: " +
            // tn.getPlayersCadic().size() + "\nPhe Fide: " + tn.getPlayersFide().size();
            // }
            ...
            // this.createOtherMenu(player, ConstNpc.REIN_TRANH_NGOC, // Xuân lol dm béo im
            // mồm
            ...
            // ký vào lúc " + TimeUtil.ShowTime(TranhNgoc.HOUR_REGISTER,
            // TranhNgoc.MIN_REGISTER) + "\nSự kiện sẽ bắt đầu vào " +
            // TimeUtil.ShowTime(TranhNgoc.HOUR_OPEN, TranhNgoc.MIN_OPEN) + " và kết thúc
            // vào " + TimeUtil.ShowTime(TranhNgoc.HOUR_CLOSE, TranhNgoc.HOUR_CLOSE));
            // break;
            // case 1:// Shop
            // ShopService.gI().openShopSpecial(player, this, ConstNpc.SHOP_CHIEN_LUC, 0,
            // -1);
            // break;
            // case 2:
            ...
            // Service.getInstance().sendThongBao(player, "Đăng ký vào phe Cadic thành
            // công");
            // } else {
            // boolean registerStatus =
            // ServerManager.gI().getTranhNgocManager().register(player, true);
            // if (registerStatus) {
            // Service.getInstance().sendThongBao(player, "Đăng ký vào phe Cadic thành
            // công");
            // } else {
            // if (ServerManager.gI().getTranhNgocManager().numOfTranhNgoc() >= 25) {
            // Service.getInstance().sendThongBao(player, "Sự kiện đang quá tải, vui lòng
            // tải lại xong!");
            // } else {
            // tranhNgoc = new TranhNgoc();
            // tranhNgoc.addPlayersCadic(player);
            ...
            // Service.getInstance().sendThongBao(player, "Đăng ký vào phe Fide thành
            // công");
            // } else {
            // boolean registerStatus =
            // ServerManager.gI().getTranhNgocManager().register(player, false);
            //
            // if (registerStatus) {
            // Service.getInstance().sendThongBao(player, "Đăng ký vào phe Fide thành
            // công");
