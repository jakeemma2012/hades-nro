        ...
        try {
            switch (tempId) {
                case ConstNpc.MONAITO:
                    npc = new MonaiTo(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.HIT:
                    npc = new Hit(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.URON_TRU:
                    npc = new UronTru(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.HANG_NGA:
                    npc = new HangNga(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.PHUOCTV:
                    npc = new NpcPhuocTv(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.TOP1:
                    npc = new NpcTop1(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.BLACKGOKU:
                    npc = new BlackGuku(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.XE_NUOC_MIA:
                    npc = new XeNuocMia(mapId, status, cx, cy, tempId, avartar);
                    break;
                case ConstNpc.BONGBANGOLD:
                    npc = new BongBangGOLD(mapId, status, cx, cy, tempId, avartar);
                    break;
                        ...
                        public void openBaseMenu(Player player) {
                            if (canOpenNpc(player)) {
                                this.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                        "Xin chào? Cậu đang làm gì ở đây thế", "NP", "Đóng");
                            }
                        }
            ...
            public void confirmMenu(Player player, int select) {
                switch (player.iDMark.getIndexMenu()) {
                    case ConstNpc.CONFIRM_DIALOG:
                        ...
                        break;
                    case ConstNpc.MENU_SKH:
                        UseItem.gI().UseHopThanLinh1400(player, select);
                        break;
                    case 9998:
                        switch (select) {
                            case 0:
                                InventoryService.gI().subQuantityItemsBag(player, (short) 1395, 500);
                                InventoryService.gI().sendItemBags(player);
                                Service.getInstance().sendThongBao(player, "Đổi thành công Đệ tử Zamas");
                                if (player.pet != null) {
                            ...
                            case 1:
                                InventoryService.gI().subQuantityItemsBag(player, (short) 1395, 500);
                                InventoryService.gI().sendItemBags(player);
                                Service.getInstance().sendThongBao(player, "Đổi thành công Đệ tử Black");
                                if (player.pet != null) {
                    ...
                    case ConstNpc.MENU_USE_ITEM_BLACK_GOKU:
                        switch (select) {
                            case 0:
                                ...
                                PetService.gI().createKidBuPet(player, player.gender);
                    ...
                    case ConstNpc.REVENGE:
                        if (select == 0) {
                            Service.getInstance().sendThongBaoOK(player, "Đang bảo trì nhie !");
                    ...
                    case ConstNpc.SUMMON_ICE_SHENRON:
                        ...
                        break;
                    case ConstNpc.SUMMON_TORONBO:
                        if (select == 0) {
                            SummonDragon.gI().summonToronbo(player);
                        }
                        break;
                    ...
                    case ConstNpc.MENU_ADMIN:
                        switch (select) {
                            ...
                            case 5:
                                if (player.pet == null) {
                                    PetService.gI().createNormalPet(player, (byte) 0);
                                }
                                break;
                        }
                        break;
        ...
        switch (Manager.EVENT_SEVER) {
            ...
            case 1:// hlw
                switch (select) {
                    ...
                    // } else {
                    // Service.getInstance().sendThongBao(player,
                    // "Vui lòng chuẩn bị x3 Hộp Ma Quỷ, x3 Vé đổi Kẹo và x3 Giỏ kẹo để đổi vật phẩm
                    // sự kiện");
                    // }
                    // } else {
                    // Service.getInstance().sendThongBao(player, "Hành trang đầy.");
