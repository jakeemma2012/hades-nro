...
public abstract class Npc implements IAtionNpc {
    ...
    // public void createOtherMenu(Player player, int indexMenu, String npcSay,
    // ImageMenu... menuSelect) {
    // Message msg;
    // try {
    // player.iDMark.setIndexMenu(indexMenu);
    // msg = new Message(32);
    ...
    // msg.writer().writeUTF(npcSay);
    // msg.writer().writeByte(menuSelect.length);
    // for (var menu : menuSelect) {
    // msg.writer().writeInt(menu.iconID);
    // msg.writer().writeUTF(menu.menu);
    // msg.writer().writeShort(menu.X);
    // msg.writer().writeShort(menu.Y);
    // }
    // player.sendMessage(msg);
    // msg.cleanup();
    // } catch (Exception e) {
        ...
        if (canOpenNpc(player)) {
            ...
            try {
                if (baseMenu != null) {
                    ...
                    msg = new Message(32);
                    msg.writer().writeShort(tempId);
                    msg.writer().writeUTF("GAME TÔI KHÔNG CÓ TÍNH NĂNG NÀY CHƠI CÁI KHÁC ĐI");
                    msg.writer().writeByte(1);
