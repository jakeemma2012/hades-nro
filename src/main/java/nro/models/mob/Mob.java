...
public class Mob {
    ...
    public synchronized void injured(Player plAtt, long damage, boolean dieWhenHpFull) {
        if (!this.isDie()) {
            ...
            if (!dieWhenHpFull) {
                ...
                if ((this.tempId == 0 || this.tempId == 90)&& damage > 10) {
                    damage = 10;
                }
            }
    ...
    public void update() {
        if (this.isDie()) {
             if (zone.map.mapId == 27 && PhubanMabu.gI().isAlive == true && PhubanMabu.gI().isTimePhuBan()) {
                Boss f = BossManager.gI().getBossById(BossFactory.OMEGA_PLUS);
                if (f != null && f.zone != null) {
                    if (zone.zoneId == f.zone.zoneId) {
                        return;
                    }
                }
            }
            if (!(zone instanceof ZSnakeRoad)) {
                if ((zone.map.type == ConstMap.MAP_NORMAL
                        ...
                        && (tempId != ConstMob.HIRUDEGARN) && Util.canDoWithTime(lastTimeDie, 2000)) {
    ...
    public void attackPlayer() {
        if (!isDie() && !effectSkill.isHaveEffectSkill() && (tempId != 0 && tempId != 90)) {
