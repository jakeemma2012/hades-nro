...
public class ArrietyDrop {
    ...
    public static Item randomCS_DKHTL(int itemId, int typeOption, byte gender) {
        ...
        switch (gender) {
            case 0:
                if (doTD[0] == itemId) {
                    it.itemOptions
                            .add(new ItemOption(47, Util.highlightsItem(gender == 2, new Random().nextInt(150) + 700))); // áo
                                                                                                                         // từ
                                                                                                                         // 1800-2800
                                                                                                                         // giáp
                }
                if (doTD[1] == itemId) {
                    it.itemOptions
                            .add(new ItemOption(22, Util.highlightsItem(gender == 0, new Random().nextInt(15) + 45))); // hp
                                                                                                                       // 85-100k
                }
                if (doTD[2] == itemId) {
                    it.itemOptions
                            .add(new ItemOption(0, Util.highlightsItem(gender == 2, new Random().nextInt(500) + 4500))); // 8500-10000
                }
                if (doTD[3] == itemId) {
                    it.itemOptions
                            .add(new ItemOption(23, Util.highlightsItem(gender == 1, new Random().nextInt(15) + 45))); // ki
                                                                                                                       // 80-90k
                }
                break;
            case 1:
    ...
    public static ItemMap DropItemSetKichHoat(Player player, int soluong, int x, int y) {
        Item itemkichhoat = ArrietyDrop.randomCS_DKH(ArrietyDrop.list_item_SKH[player.gender][Util.nextInt(0, 4)],
                (byte) 0, player.gender);
        ItemMap item = new ItemMap(player.zone, itemkichhoat.template.id, soluong, Util.nextInt((x - 50), (x + 50)), y,
                player.id);
    ...
    public static ItemMap DropItemReWardDoTL(Player player, int soluong, int x, int y) {
        Item itemHuyDiet = RewardService.randomCS_DHD(ArrietyDrop.list_do_than_linh[player.gender][Util.nextInt(0, 2)],
                player.gender);
        ItemMap item = new ItemMap(player.zone, itemHuyDiet.template.id, soluong, Util.nextInt((x - 50), (x + 50)), y,
                player.id);
        item.options = itemHuyDiet.itemOptions;
        return item;
    }

    public static ItemMap DropItemRewardNormal(Player player, int soluong, int x, int y) {
        ItemMap item = new ItemMap(player.zone,
                (short) ConstItem.LIST_ITEM_CLOTHES[Util.nextInt(3)][Util.nextInt(0, 4)][Util.nextInt(9, 11)], soluong,
                Util.nextInt((x - 50), (x + 50)), y, player.id);
        RewardService.gI().initBaseOptionClothesMap(item);
        return item;
    }

    public static void initialOptionAnKichHoat(ItemMap item) {
        item.options.add(new ItemOption(Util.nextInt(34, 36), 0));
    }
