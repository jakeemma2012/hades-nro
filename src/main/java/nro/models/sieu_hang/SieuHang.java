...
public class SieuHang {
    ...
    public static BossData getBossDataFromPlayer(Player pl) {
        List<Skill> skills = pl.playerSkill.skills.stream()
                .filter(s -> s != null && (s.point > 0 || s.template.id == Skill.KHIEN_NANG_LUONG)).toList();
        ...
        BossData boss = new BossData(
                ...
                pl.nPoint.getDameAttack(false, false), // dame
                new long[][] { { pl.nPoint.hpMax } }, // hp
    ...
    public void endChallenge(Player plWin) {
        ...
        if (player != null && player.zone != null && player.zone.map.mapId == ConstMap.DAI_HOI_VO_THUAT_113) {
            Util.setTimeout(() -> {
                ChangeMapService.gI().changeMapNonSpaceship(player, ConstMap.DAI_HOI_VO_THUAT_113, player.location.x,
                        360);
            }, 500);
        }
        if (boss != null) {
