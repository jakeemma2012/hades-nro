...
public class EffectSkin {
    ...
    private static final String[] textOdo = new String[] {
            "Nóng quá", "Ối ối?? Cái gì thế", "Cái khí hậu khắc nghiệt gì thế này!!",
            "Nực quá", "Chạy ngay thôi!", "Điên rồ quá!"
    };
    private static final String[] textXinbato = new String[] {
            "Phân tâm quá !!!", "Tránh ra đi !", "Không có nước đâu !"
    };
    ...
    private long lastTimePhuXinbato;
    private long lastTimeGohan;
    ...
    public long lastTimeGodzila;
    public boolean isDownPoint;
    private long lasttimeDownPoint;
    ...
    private void updateBunma() {
        try {
            if (this.player.setClothes.ctBunmaXecXi != -1 && this.player.zone != null
                    && Util.canDoWithTime(lastTimeXinhQua, 10_000)) {
                List<Player> players = new ArrayList<>();
                List<Player> playersMap = this.player.zone.getNotBosses();
                for (Player pl : playersMap) {
                    if (!this.player.equals(pl) && !pl.isBoss && !pl.isDie()
    ...
    private void updateXinbato() {
        if (this.player.nPoint.wearingXinbaTo) {
            if (Util.canDoWithTime(lastTimePhuXinbato, 10000)) {
                List<Player> players = new ArrayList<>();
                ...
                for (Player pl : players) {
                    if (pl.isPl()) {
                        pl.effectSkin.isXinbato = true;
                        pl.effectSkin.lastTimeXinbato = System.currentTimeMillis();
                        Service.getInstance().chat(pl, textXinbato[Util.nextInt(0, textXinbato.length - 1)]);
                    }
                }
                this.lastTimePhuXinbato = System.currentTimeMillis();
            }
        }
    }

    public void UpdateGohan() {
        if (Util.canDoWithTime(lastTimeGohan, 5000)) {
            List<Player> players = new ArrayList<>();

            List<Player> playersMap = this.player.zone.getNotBosses();
            for (Player pl : playersMap) {
                if (!this.player.equals(pl) && !pl.isBoss && !pl.isDie()) {
                    if (pl.inventory.itemsBody.get(5) != null) {
                        if (pl.inventory.itemsBody.get(5).template.id == 883) {
                            players.add(pl);
                        }
                    }
                }
            ...
            this.lastTimeGohan = System.currentTimeMillis();
        }
    }

    private void RemoveXinbato() {
        if (isXinbato) {
            if (Util.canDoWithTime(lastTimeXinbato, 3000)) {
                isXinbato = false;
                Service.getInstance().chat(player, "Phù ! Thoát được Xinbato rồi !!!");
            }
        }
        if (isDownPoint) {
            if (Util.canDoWithTime(lasttimeDownPoint, 5000)) {
                isDownPoint = false;
                Service.getInstance().chat(player, "Tròi sáng lại rồi !!...");
                Service.getInstance().point(player);
            }
        }
    }
    ...
    private void updateGodZilla() {
        try {
            if (this.player.nPoint.isDoNgocRong) {
                if (Util.canDoWithTime(lastTimeGodzila, 5000)) {
                    List<Player> players = new ArrayList<>();
                    List<Player> playersMap = this.player.zone.getNotBosses();
                    for (Player pl : playersMap) {
                        if (!this.player.equals(pl) && pl.isPl() && !pl.isDie()
                                && Util.getDistance(this.player, pl) <= 200) {
                            players.add(pl);
                        }
                    }
                    for (Player pl : players) {
                        if (!pl.itemTime.isBanhTrungThu1Trung) {
                            pl.effectSkin.isDownPoint = true;
                            pl.effectSkin.lasttimeDownPoint = System.currentTimeMillis();
                            Service.getInstance().point(pl);
                            ItemTimeService.gI().sendItemTime(pl, 11730, 5000 / 1000);
                        }
                    }
                    this.lastTimeGodzila = System.currentTimeMillis();
                }
            }
        } catch (Exception e) {
            Log.error("updateDrabula() err" + e.getMessage());
