...
public class Pet extends Player {
    ...
    public void fusionFollowType(boolean porata, int Type) {
        if (this.isDie()) {
            Service.getInstance().sendThongBao(master, "Không thể thực hiện");
            return;
        }
        if (Util.canDoWithTime(lastTimeUnfusion, TIME_WAIT_AFTER_UNFUSION)) {
            if (porata) {
                master.fusion.typeFusion = (byte) Type;
            } else {
    ...
    private void fusionEffect(int type) {
        Service.getInstance().sendFusionEffect(master, type);
    }
        ...
        try {
            ...
            switch (this.playerSkill.skillSelect.template.id) {
                case Skill.DICH_CHUYEN_TUC_THOI:
                    ...
                    return false;
                case Skill.HUYT_SAO:
                    if (this.effectSkill.tiLeHPHuytSao == 0 && SkillService.gI().canUseSkillWithCooldown(this)
                            && SkillService.gI().canUseSkillWithMana(this)) {
                        SkillService.gI().useSkill(this, null, null);
                        return true;
                    }
                    return false;
                case Skill.TU_SAT:
                    if (!this.playerSkill.prepareTuSat && SkillService.gI().canUseSkillWithCooldown(this)
                            && SkillService.gI().canUseSkillWithMana(this)) {
                        SkillService.gI().useSkill(this, null, null);
                        return true;
                    }
    ...
    public short getAvatar() {
        switch (this.typePet) {
            ...
            case ConstPet.BU_GAY:
                return 1352;
            case ConstPet.VIDEL:
                if (nPoint.power > 80_000_000_000L) {
                    return 1434;
                } else {
                    return 903;
                }
    ...
    public short getHead() {
        if (effectSkill.isMonkey) {
            ...
            return 297;
        } else if (this.typePet == ConstPet.BU_GAY && !this.isTransform) {
            return 1352;
            ...
            return 838;
        } else if (typePet == ConstPet.KIDBU && !this.isTransform) {
            return 424;
    ...
    public void transform() {
        switch (this.typePet) {
            ...
            case ConstPet.BU_GAY:
                this.isTransform = !this.isTransform;
                Service.getInstance().Send_Caitrang(this);
                Service.getInstance().chat(this, "Buuuuuuuuuuuuuuuuuuuu");
                break;
            case ConstPet.VIDEL:
                this.isTransform = !this.isTransform;
                Service.getInstance().Send_Caitrang(this);
