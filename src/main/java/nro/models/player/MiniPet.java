...
public class MiniPet extends Player {
    ...
    private void followMaster(int dis) {
        ...
        if (Math.sqrt(Math.pow(mX - this.location.x, 2) + Math.pow(mY - this.location.y, 2)) >= dis) {
            ...
            this.location.y = mY;
            if (master.inventory.itemsBody.get(9).template.id == 1408) {
                EffectMapService.gI().sendEffectMapToPlayer(this, 0, 3, 5, this.location.x - 24, this.location.y + 24, 1);
                EffectMapService.gI().sendEffectMapToAllInMap(this.zone, 0, 3, 5, this.location.x - 24, this.location.y + 24, 1);
            }
