...
//        }
    ...
    public void changeMap(Player player, byte index) {
        try {
            long now = System.currentTimeMillis();
            if (now > TIME_OPEN && now < TIME_CLOSE) {
                int mapId = player.mapBlackBall.get(index).map.mapId;
                if (mapId == 90 || mapId == 89) {
                    Service.getInstance().sendThongBao(player, "Hiện tại Ngọc rồng Sao đen 5 và 6 sao đang bận !\n"
