    ...
    private void consign(Player player, short itemID, byte monneyType, int money, int quantity) {
        ...
        if (monneyType == 0) {
            if (money <= 1 || money >= 9999) {
                Service.getInstance().sendThongBao(player, "Vui lòng ký gửi trên 1 đơn vị, đến 9999 đơn vị");
                show(player);
                return;
            }
        } else {
        ...
        for (ConsignmentItem item : list) {
            if (item.getConsignID() == itemID && monneyType == monneyType && money == money) {
                if (player.playerTask.taskMain.id < 19) {
                    Service.getInstance().sendThongBao(player, "Bạn phải hoàn thành nhiệm vụ Nappa");
                    show(player);
                    return;
                }
                if (!player.getSession().actived) {
                    Service.getInstance().sendThongBao(player, "Bạn chưa phải là thành viên của " + Manager.DOMAIN);
                    show(player);
                    return;
                }
                if (item.isSold()) {
                ...
                if (monneyType >= 0) {
                    if (ItemService.gI().SubThoiVang(player, (short) money)) {
                        ...
                        Service.getInstance().sendThongBao(player,
                                "Bạn không đủ thỏi vàng để mua vật phẩm này!");
                        isBuy = false;
                        show(player);
                        return;
                        // openShopKyGui(pl);
    ...
    public void getMoney(Player player, short itemID) {
        for (ConsignmentItem item : list) {
            if (item.template.id == itemID && item.getConsignorID() == player.id && item.isSold()) {
                int ck = 1;
                if (item.getPriceGold() * 10 / 100 >= 1) {
                    ck = item.getPriceGold() * 10 / 100;
                }
                int quanlityTv = item.getPriceGold() - ck;
                ...
                Service.getInstance().sendThongBaoOK(player,
                        "Bạn đã bán vật phẩm thành công và nhận được " + quanlityTv
                        + " thỏi vàng (phí 10%)");
//                show(player);
