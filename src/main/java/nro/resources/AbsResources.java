...
public abstract class AbsResources {
    ...
    public void initIBN() {
        String json = readString("imgByname.json");
        if (!json.equals("")) {
    ...
    public void initDataVersion() {
        ...
        for (int i = 0; i < 4; i++) {
            File folder = new File(this.folder, "Data_File");
    ...
    public void initBGSmallVersion() {
        try {
            ...
            for (int i = 0; i < 4; i++) {
                File file = new File(this.folder, "/BackGround/imageItemBG/x" + (i + 1) + "/");
    ...
    public void initSmallVersion() {
        try {
            ...
            for (int i = 0; i < 4; i++) {
                File file = new File(this.folder, "/ImageIcon/x" + (i + 1) + "/");
                File[] files = file.listFiles();
                int max = 0;
                for (File f : files) {
                    String name = f.getName();
    ...
    protected void setFolder(String folder) {
        this.folder = new File("AbsReSource");
    }
    ...
    public byte[] getRawIconData(int zoomLevel, int iconID) {
        return readAllBytes("/ImageIcon/x" + zoomLevel + "/" + iconID + ".png");
    }

    public byte[] getRawBGData(int zoomLevel, int bg) {
        return readAllBytes("/BackGround/imageItemBG/x" + zoomLevel + "/" + bg + ".png");
    }

    public byte[] getRawIBNData(int zoomLevel, String filename) {
        return readAllBytes("/ImageByName/x" + zoomLevel + "/" + filename + ".png");
    }
