...
//        }
        ...
        switch (npc.tempId) {
            ...
            case ConstNpc.BUNMA_TL:
                return (doneTask(player, ConstTask.TASK_22_0)
                        || doneTask(player, ConstTask.TASK_22_8)
            ...
            case ConstNpc.BLACKGOKU:
                return (doneTask(player, ConstTask.TASK_26_2)
        ...
        if (player != null && !player.isBoss && !player.isPet) {
            switch ((int) boss.id) {
                ...
                case BossFactory.ANDROID_14:
                    doneTask(player, ConstTask.TASK_22_6);
                    break;
                case BossFactory.ANDROID_13:
                    doneTask(player, ConstTask.TASK_22_7);
                    break;
                ...
                case BossFactory.TRUNG_UY_TRANG:
                    ...
                    break;
                case BossFactory.TRUNG_UY_XANH_LO:
                    doneTask(player, ConstTask.TASK_27_1);
                    break;
                case BossFactory.TRUNG_UY_THEP:
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_30_1:
                    npcSay(player, ConstNpc.BLACKGOKU,
                            "Ngươi thật sự là kẻ mạnh đấy !");
                    break;
                case ConstTask.TASK_31_2:
                case ConstTask.TASK_32_3:
                    npcSay(player, ConstNpc.QUY_LAO_KAME,
                            "Con đã làm rất tốt !");
                    break;
            }
            InventoryService.gI().sendItemBags(player);
    ...
    public boolean isCurrentTask(Player player, int idTaskCustom) {
        switch (idTaskCustom) {
            ...
            case ConstTask.TASK_22_7:
                return player.playerTask.taskMain.id == 22 && player.playerTask.taskMain.index == 7;
            case ConstTask.TASK_22_8:
                return player.playerTask.taskMain.id == 22 && player.playerTask.taskMain.index == 8;
            case ConstTask.TASK_23_0:
    ...
    public int getIdTask(Player player) {
        ...
        } else if (player.playerTask.taskMain.id == 22 && player.playerTask.taskMain.index == 7) {
            return ConstTask.TASK_22_7;
        } else if (player.playerTask.taskMain.id == 22 && player.playerTask.taskMain.index == 8) {
            return ConstTask.TASK_22_8;
