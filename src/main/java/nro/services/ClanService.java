...
public class ClanService {
    ...
    public List<Clan> getClans(String name) {
        ...
        if (Manager.CLANS.size() <= 20) {
            ...
            List<Clan> shuffledClans = new ArrayList<>(Manager.CLANS);
    ...
    private void createClan(Player player, byte imgId, String name) {
        if (player.clan == null) {
            ...
            if (flagBag != null) {
                if (flagBag.gold > 0) {
                    int quan = InventoryService.gI().getQuantity(player, 457);
                    if (quan >= flagBag.gold) {
                        InventoryService.gI().subQuantityItemsBag(player, 457, flagBag.gold);
                        InventoryService.gI().sendItemBags(player);
                    } else {
                        Service.getInstance().sendThongBao(player, "Bạn không đủ thỏi vàng , còn thiếu "
                                + Util.numberToMoney(flagBag.gold - quan) + " thỏi vàng");
                        return;
                    }
                }
                if (flagBag.gem > 0) {
...
//                ClanDAO.saveClan(player.clan);
                ...
                Service.getInstance()
                        .sendThongBao(player, "Đổi thành công biểu tượng " + flagBag.name);
            }
        }
    }
    ...
    public void close() {
        ...
        try {
            ...
            for (Clan clan : Manager.CLANS) {
                ...
                ps.setString(1, clan.slogan != null ? clan.slogan : "");
