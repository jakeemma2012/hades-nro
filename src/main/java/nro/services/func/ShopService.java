...
import nro.models.npc.NpcForge.ThuongDe;
...
public class ShopService {
    ...
    private ItemShop getItemShop(int shopId, int tempId) {
        ...
        switch (shopId) {
            ...
            case ConstNpc.SHOP_URON_0:
                ...
                break;
            case ConstNpc.SHOP_URON_1:
                shop = getShop(ConstNpc.URON, 1, -1);
                break;
            ...
            case ConstNpc.SHOP_BUNMA_TL_0:
                ...
                break;
            case ConstNpc.SHOP_BONG_BANG_GOLD:
                shop = getShop(ConstNpc.BONGBANGOLD, 0, -1);
                break;
            ...
            case ConstNpc.SHOP_BUNMA_LINH_THU:
                ...
                break;
            case ConstNpc.SHOP_GOKU_SSJ:
                shop = getShop(ConstNpc.GOKU_SSJ_, 0, -1);
                break;
            case ConstNpc.SHOP_NUOC_MIA:
                shop = getShop(ConstNpc.XE_NUOC_MIA, 0, -1);
                break;
            case ConstNpc.BA_HAT_MIT_BLACK:
                shop = getShop(ConstNpc.BA_HAT_MIT, 5, -1);
                break;
            ...
            for (ItemShop item : tabShop.itemShops) {
                ...
                switch (item.temp.id) {
                    ...
                    case 672:
                        ...
                        break;
                    case 1508:
                        min = (player.charms.tdNoStamina - System.currentTimeMillis()) / 60000;
                        break;
                }
                if (min > 0) {
                    item.options.clear();
    ...
    public void openShopNormal(Player player, int idNpc, int shopId, int order, int gender) {
        Shop shop = getShop(idNpc, order, gender);
        openShopType0(player, shop, shopId);
    }
        ...
        if (is != null) {
            ...
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
                ...
                if (gold != 0) {
                    ...
                    } else if ((itemExchange == 189 || itemExchange == 188)
    ...
    private void getMocRewards(Player player, byte type, int index) {
        if (index < 0 || index >= Manager.gI().itemsReward.size()) {
            return;
        }
        if (player.receive[index] < 1) {
            if (player.pointVongQuayThuongDe >= Manager.gI().mocPointItemsRewardsList.get(index)) {
                Item item = Manager.gI().itemsReward.get(index);
                switch (type) {
                    case 0: // nhận
                        if (item.isNotNullItem()) {
                            if (InventoryService.gI().getCountEmptyBag(player) != 0) {
                                ...
                                Service.getInstance().sendThongBao(player,
                                        "Bạn nhận được " + (item.template.id == 189
                                                ...
                                                : item.template.name));
                                InventoryService.gI().sendItemBags(player);
                                player.receive[index] = 1;
                                ThuongDe.OpenMocRewards(player);
                            } else {
                                Service.getInstance().sendThongBao(player, "Hành trang đã đầy");
                            }
                        } else {
                ...
                Service.getInstance().sendThongBao(player,
                        "Bạn không đủ "
                                + Util.numberToMoney(
                                        Manager.gI().mocPointItemsRewardsList.get(index) - player.pointVongQuayThuongDe)
                                + " nữa để đổi mốc " + index);
            }
        } else {
            Service.getInstance().sendThongBao(player, "Bạn đã nhận mốc " + (index) + " rồi mà !");
        }
    }
    // --------------------------------------------------------------------------
    // điều hướng mua
    ...
    public void buyItem(Player player, byte type, int tempId) {
        switch (player.iDMark.getShopId()) {
            ...
            case ConstNpc.SHOP_LEARN_SKILL:
                ...
                break;
            case ConstNpc.MOC_REWARDS:
                getMocRewards(player, type, tempId);
                break;
            default:
