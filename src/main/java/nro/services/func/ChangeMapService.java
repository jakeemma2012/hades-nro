...
public class ChangeMapService {
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (pl.isAdmin() || pl.itemTime.isSuperZone || Util.canDoWithTime(pl.lastTimeChangeZone, 10000)) {
            pl.lastTimeChangeZone = System.currentTimeMillis();
            Map map = pl.zone.map;
            if (zoneId >= 0 && zoneId <= map.zones.size() - 1) {
                Zone zoneJoin = map.zones.get(zoneId);

                if (zoneJoin != null && (zoneJoin.getNumOfPlayers() >= zoneJoin.maxPlayer)) {
                ...
                Service.getInstance().sendThongBaoOK(pl, "Không thể thực hiện");
            }
        } else {
...
//        if (isTimeSupportTask(TimeUtil.getCurrHour()) && zoneJoin != null) {
//            for (Player b : zoneJoin.getBosses()) {
//                if (((b.id == BossFactory.XEN_BO_HUNG_1 || b.id == BossFactory.XEN_BO_HUNG_2
//                        || b.id == BossFactory.XEN_BO_HUNG_HOAN_THIEN)
//                        && (TaskService.gI().getIdTask(player) < ConstTask.TASK_24_1
//                        || TaskService.gI().getIdTask(player) > ConstTask.TASK_24_3))) {
//                    Service.getInstance().sendThongBao(player,
//                            "Đang trong giờ hỗ trợ nhiệm vụ , không thể vào khu");
//                    return null;
//                }
//            }
//        }
...
//                    break;
    ...
    boolean isTimeSupportTask(int Time) {
        return switch (Time) {
            case 13, 15, 17, 20, 22, 24 ->
                true;
            default ->
                false;
        };
