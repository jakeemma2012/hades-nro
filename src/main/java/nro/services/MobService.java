...
public class MobService {
    ...
    public void sendMobStillAliveAffterAttacked(Mob mob, long dameHit, boolean crit, Player player) {
        Message msg;
        try {
            if (player != null) {
                player.checkHutMau();
    ...
    public long mobAttackPlayer(Mob mob, Player player) {
        ...
        if(player.isPl() && player.getSession().isAdmin){
            dameMob = 1;
    ...
    public void initMobDoanhTrai(Mob mob, long point) {
        mob.point.hp = mob.point.maxHp = (point / 10);
        mob.point.dame = mob.point.dame =  (point / 200);
    }
